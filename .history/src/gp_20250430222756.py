...
def gp_aggr_count(config=None):
    """
    Gaussian Process aggregation model for spatial *count* data.

    This is the full model used for inference on dengue case counts, incorporating:
    - Aggregated latent GP process (from gp_aggr)
    - Covariates like population density, HDI, and urbanicity
    - A linear predictor with fixed effects
    - Optional observation for count data
    """
    if config is None:
        config = {}
