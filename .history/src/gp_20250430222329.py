...
def gp_aggr(config=None):
    ...
    kernel_length_prior = config.get('kernel_length', dist.InverseGamma(4, 1)) # Section: Prior for kernel length
    kernel_var_prior = config.get('kernel_var', dist.LogNormal(0, 0.1)) # Section: Prior for kernel variance
