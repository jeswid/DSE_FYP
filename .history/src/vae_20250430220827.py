...
def vae_model(batch, hidden_dim, z_dim, vae_var):
    ...
    # Likelihood: observed data ~ N(gen_loc, vae_var)
    obs = numpyro.sample(
        "obs",
...
def vae_guide(batch, hidden_dim, z_dim):
    ...
    """
    batch = jnp.reshape(batch, (batch.shape[0], -1)) # Flatten to 2D
    batch_dim, input_dim = jnp.shape(batch)

    # Load encoder neural network
    ...
    # Forward pass to get parameters of q(z|x)
    z_loc, z_std = encode(batch) #mu : (num_samples, z_dim), sigma2 : (num_samples, z_dim)

    # Sample a value z based on mu and sigma (variational posterior)
